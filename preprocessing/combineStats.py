...
def combine_stats(all_mean, all_var, sampls, N):
    ...
    for channel in range(0,len(all_mean)):
        for exp in range(0,N):
            ...
            # mean
            product_mean_sampl = sampls.iloc[channel,exp]*all_mean.iloc[channel,exp]
            
            numerator_mean = numerator_mean + product_mean_sampl
            total_sampl = total_sampl + sampls.iloc[channel,exp]
            
            # variance
            q = (sampls.iloc[channel,exp] - 1)*all_var.iloc[channel,exp] + sampls.iloc[channel,exp]*(all_mean.iloc[channel,exp]**2)
    ...
    savehere = Path().resolve()     # current directory
    stats = pd.DataFrame({'mean': combined_mean, 'standard_dev': combined_std})   
    np.save(os.path.join(savehere, 'combined_stats'), stats)  # combined mean and standard deviation from all experiments
